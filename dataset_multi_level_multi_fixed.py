...
class SphericalDataset(Dataset):
    ...
    def get_moving(self, sub_id, ico_level):
        ...
        if self.seg is True:
            sub_dir_label = os.path.join(self.dir_result, sub_id, 'label')
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            index_fixed = np.random.randint(0, len(self.sub_dirs))
            sub_id_fixed = self.sub_dirs[index_fixed].strip()
            fixed = list()
        else:
        ...
        # fixed = self.get_fixed()
        for ico_level in self.ico_levels:
            sulc_moving, curv_moving, xyz_moving, faces_moving, seg_moving = self.get_moving(sub_id, ico_level)
            movings.append([sulc_moving, curv_moving, xyz_moving, faces_moving, seg_moving])
            if self.is_train:
                sulc_fixed, curv_fixed, xyz_fixed, faces_fixed, seg_fixed = self.get_moving(sub_id_fixed, ico_level)
                fixed.append([sulc_fixed, curv_fixed, xyz_fixed, faces_fixed, seg_fixed])
